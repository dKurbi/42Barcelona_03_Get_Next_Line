...
/*   Updated: 2024/01/18 16:29:53 by dkurcbar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (BUFFER_SIZE <= 0 || read(fd, NULL, 0) < 0)
		return (my_free(&storage[fd], NULL, NULL));
	if (storage[fd])
		exit_code = check_storage(&storage[fd], &rtn);
	if (exit_code == OK)
		exit_code = read_line(fd, &line);
	if (exit_code == OK)
		exit_code = manage_exit(&rtn, &line, &storage[fd]);
	if (exit_code == ERROR)
		return (my_free(&storage[fd], &rtn, NULL));
	return (rtn);
}
